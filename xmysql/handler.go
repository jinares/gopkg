...
type (
	SyncDataHandler func(row RowData) error
)

func NewDumpHandler(c *canal.Canal, f SyncDataHandler) *SyncDumpEventHandler {
...
func (h *SyncDumpEventHandler) getData(tableInfo *schema.Table, values []string) map[string]interface{} {
	...
	for i, v := range values {
		if v == "NULL" {
			...
			//|| tableInfo.Columns[i].Type == schema.TYPE_MEDIUM_INT
